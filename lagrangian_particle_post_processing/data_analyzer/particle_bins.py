...
class ParticleBinCell:
    ...
    def __init__(self, ParticleDiameter=None, PartsPerParcel=None, EleCount=0):
        self.ParticleDiameters = [] if ParticleDiameter is None else  ParticleDiameter[:] #String data 
        self.ParticlesPerParcel = [] if PartsPerParcel is None else PartsPerParcel[:] #String data
    ...
    def add_data(self, diameter, particles_per_parcel):
        ...
        self.ParticlesPerParcel.append(particles_per_parcel)
        self.NumElements = self.NumElements + 1

    def print_data(self):
        logger.info("Data set has %d elements. Output below is Nx2 array: Diameter and Particles Per Parcel"%(self.NumElements))
        logger.info("Number of elements in ParticlesPerParcel list: %d"%(len(self.ParticlesPerParcel)))
        logger.info('Diameter  ->  Particles Per Parcel')
        for i in range(0, self.NumElements):
            logger.info("%10.6E\t%10.6E"%(float(self.ParticleDiameters[i]), float(self.ParticlesPerParcel[i])))

    def check_data_length(self):
        if self.NumElements != len(self.ParticleDiameters):
            logger.error("ERROR: Mis-match between actual amount of data & expected amount of data contained in object")
            logger.error("Object contains %d elements, but thinks it contains %d elements"%(len(self.ParticleDiameters), self.NumElements))
        else:
            logger.info("Data Check Passed. Object has %d elements and thinks it contains %d elements"%(len(self.ParticleDiameters), self.NumElements))
    ...
    def custom_bins(self, UserDiameterBins):
        ...
        for i in range(0, self.NumElements):
            ...
            if Found == False:# A value in the original data set was not able to be placed into the bins
                logger.warning("Warning: The value: %10.6E  in the original data set was unable to be placed into the user defined bins"%(float(self.ParticleDiameters[i])))
        ...
        if abs(CheckSum1 - CheckSum2) >= 1e-6:
            ...
            logger.warning("Magnitude of Error: %10.6E"%(abs(CheckSum1 - CheckSum2)))
    ...
    def __add__(self, other):
        ...
        if other.NumElements > 0:  #Only add the new data if it actually exists
            ...
            if abs(CheckSum1 - CheckSum2) >= 1e-9:
                logger.error("ERROR - Addition process has lost data! Discrepancy is: %10.6E"%(abs(CheckSum1 - CheckSum2)))
