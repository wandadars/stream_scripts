...
#       for k,Residual_Name in residual_names.items():
...
for i,Residual_name in residual_names.items():
...
for i,Residual_name in residual_names.items(): #Loop for the different variables
...
for i,Residual_name in residual_names.items(): #Loop for the different variables
    for j in range(0,len(timesteps)):
        epsilon = 1e-15
        if float(timesteps[j].get_ending_residual(i)) < epsilon:
            Residual_drop = 0
        else:
