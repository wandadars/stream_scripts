...
def SauterMeanDiameter(ParticleData,N_Parcels,BinFlag):

	import numpy as np

	#Note that ParticleData is a 2D array of particle diameters & number of particles
	
	#Check to make sure that more than 1 entry exists, otherwise return the diameter
	if(N_Parcels == 1):
		return (ParticleData[0]**3)/(ParticleData[0]**2)
